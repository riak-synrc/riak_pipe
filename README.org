...
A diagram recording the input/output/etc. messages passed between
source/vnode/fitting/worker/sink is included in the file
=riak_pipe_flow.msc=.  The comments at the top of that file describe
how to reder it to an image using Mscgen.
